...
//Sbox 8
    ...
    public int sBoxCalculation(int inputBit, int boxNum) {
        ...
        StringBuilder binary = new StringBuilder(String.valueOf(inputBit));
        // If there is a 0 at the front, Integer.toBinaryString(inputBit) returns result without it
        while (countDigit(inputBit) < 6) {
            binary.insert(0, '0');
            if (binary.length() == 6)
                break;
        }
        ...
        String x = String.valueOf(binary.charAt(0)) + binary.charAt(length - 1);

        StringBuilder y = new StringBuilder();
        for (int i = 1; i <= length - 2; i++) {
            y.append(binary.charAt(i));
        }
    ...
    public static void main(String[] args) {
        Alt testObject = new Alt();
        System.out.println(testObject.sBoxCalculation(11101, 3));
        System.out.println(testObject.sBoxCalculation(100111, 1));
        System.out.println(testObject.sBoxCalculation(11001, 3));
        System.out.println(testObject.sBoxCalculation(110011, 5));
        System.out.println(testObject.sBoxCalculation(111, 7));
